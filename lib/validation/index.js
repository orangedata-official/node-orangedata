const { isInt, isEmail, isDecimal, isDate, isISO8601 } = require('validator');
...
const round10 = (value) => decimalAdjust('round', value, -2);


const totalPositions = (positions = []) => Number(positions.reduce(
  (sum, position) => sum + round10(Number((position.price * position.quantity).toFixed(10))), 0,
));
...
module.exports.requiredIf = function requiredIf(callback) {
  return (value, data) => (
    callback(data, value) && isEmpty(value)
      ? 'Требует заполнения'
      : false
  );
};
...
module.exports.date = function date(value) {
  if (
    !isEmpty(value)
    && !isDate(value, { format: 'DD/MM/YYYY', delimiters: ['/', '.'] })
    && /\//gi.test(value)
  ) {
    return 'Должно быть строкой в формате ДД.ММ.ГГГГ';
  }
  return false;
};

module.exports.isIso8601 = function isIso8601(value) {
  if (!isEmpty(value) && !isISO8601(value)) {
    return 'Должно быть в формате ISO8601';
  }
  return false;
};
...
module.exports.amounts = function amounts({ payments = [] } = {}, { positions = [] } = {}) {
  ...
  const totalPos = totalPositions(positions);
  if ((totalPos - totalPayments(payments)).toFixed(2) > MIN_DIFF) {
    return 'Сумма всех типов оплаты меньше итога чека.';
  }
  if ((totalPaymentsWithoutCash - totalPos).toFixed(2) > MIN_DIFF) {
    return 'Вносимая безналичной оплатой сумма больше итога чека.';
  }
  return false;
};

// Проверка кода товарной номенклатуры
module.exports.validateNomenclatureCode = (value) => {
  if (value === undefined || value === null) return null;
  const isBase64 = /^([0-9a-zA-Z+/]{4})*(([0-9a-zA-Z+/]{2}==)|([0-9a-zA-Z+/]{3}=))?$/.test(value);
  return isBase64 ? false : 'Должно быть строкой, содержащий base64 кодированный массив';
};
...
module.exports.boolean = function boolean(value) {
