...
const {
  ...
  maxPositionsTotal, maxPaymentsTotal,
  inn, phone, amounts, // specific validators
  or, arrayOf, // wrappers
} = require('./index');
...
const order = {
  ...
  content: createValidator({
    ...
    positions: [required, maxPositionsTotal(MAX_MONEY_AMOUNT), arrayOf(position)],
    checkClose: [amounts, createValidator({
      payments: [required, maxPaymentsTotal(MAX_MONEY_AMOUNT), arrayOf(payment)],
